...
//when new poll for directions change 
...
//just move the snake when no poll for directions 
void list::slide()
{ 
   struct link *temp; 
	temp=(struct link *)malloc(sizeof(struct link));
...
//New right side node generation 
...
//New bottom side node generation
...
//Remove the last node for shifting the snake 
...
//draw the list also cheak for self beat 
void list::draw()
{ 
	struct link *temp;
...
//To delete the last node or not
...
//On noting the snake beat itself do the follow
void list::onbeat() 
{
   while(1) 
	{
...
//follow the procedure after eating food
void list::getfood() 
{
   flg='s'; 
	x=y=-1;
...
void list::endscreen() 
{
   ...
   gotoxy(40,15); 
	cout<<score;
   outtextxy(240,140,"G A M E   S C O R E"); 
	outtextxy(260,340,"Developed by");
   outtextxy(187,385,"Email : Ali Mihan doost @ msn.com"); 
	setcolor(11);
   outtextxy(220,365,"A L I  M i h a n D o o s t"); 
...
void main()
{
//intialize the graphics drivers
	...
	setcolor(7);
	outtextxy(190,MAX_Y+62,"Developed by  :  Ali Mihandoost");
